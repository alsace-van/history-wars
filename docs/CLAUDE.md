...
13. **`cubeKey` est format axial 2 composantes ("q,r"), JAMAIS splitter en 3**. Cause : `cubeKey(c)` retourne `${q},${r}` (s deductible via -q-r). Tout code qui fait `k.split(',')` en destructurant 3 elements obtient `s = Number(undefined) = NaN`. Conséquence : `cubeDistance(dest, ...)` retourne NaN, comparaisons numériques silencieusement fausses (NaN > x = false, NaN < x = false), score de l'algo reste à zéro, aucune erreur, aucun crash. Fix : utiliser `parseCubeKey(k)` qui recalcule s = -q-r. Detection : algo hex (IA, pathfinding, scoring) qui ne fait jamais le bon choix, scores tous égaux à 0, pas de crash. Grep `cubeKey.*split\|split.*sStr` pour repérer d'autres occurrences. Vu session 23 (Phase 4) : bot pickait `hold` 5× par tour car tous les `dest.s = NaN` dans les moves enumérés (`src/engine/ai/picker.ts` v1.0 + mirror Deno).
